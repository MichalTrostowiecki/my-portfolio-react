
// This component it's rendered inside AboutPage
const AboutMe = () => {
    return (
		<div className="p-4 mx-auto flex flex-col items-center mb-10">
			<h2 className="underline decoration-cyan-400 text-center mx-auto p-5 w-3/4 text-3xl font-semibold
                leading-relaxed text-white underline-offset-8">
				About Me
			</h2>
			<div>
				<p className="text-center max-w-full md:w-3/4 p-5 text-xl font-semibold leading-relaxed text-white/85 mx-auto">
					Transitioning from personal training into web development, I bring a unique blend of client-focused skills and technical expertise.
					My dedication to this new path is marked by completing an EDX Front-End Development Bootcamp,
					where I honed my abilities in HTML, JavaScript, CSS, and React.
					As an empathetic communicator and adaptable problem-solver,
					I’m seeking a developer role to apply my skills in a dynamic team,
					driving projects that combine innovative tech solutions with client-centered design.
					Eager to contribute and grow in a tech environment,
					I am ready to leverage my diverse background and training for impactful web development.
				</p>
			</div>
        </div>
    )
}

export default AboutMe;
